...
export default function BarcelonaLanding({ locale, servicePath }: { locale: string; servicePath?: string }) {
  ...
  const marketBase = marketBasePath(locale, "barcelona");
  ...
  const estimateHref = marketEstimateHref(locale, "barcelona", servicePath ? `service=${encodeURIComponent(servicePath)}` : "");
  ...
  const serviceJsonLd = {
    ...
    "@type": "Service",
    name: title,
    url: localizedUrl(locale, `barcelona${servicePath ? `/${servicePath}` : ""}`),
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Barcelona", item: localizedUrl(locale, "barcelona") },
      ...(servicePath ? [{ "@type": "ListItem", position: 3, name: serviceName, item: localizedUrl(locale, `barcelona/${servicePath}`) }] : []),
    ],
  };
  ...
  return (
    <main className="bg-white pb-20 text-neutral-950 md:pb-0">
      ...
      <section className="relative overflow-hidden border-b border-yellow-200 bg-[radial-gradient(circle_at_top_right,_#fef08a_0,_#fff_42%)]">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:py-20">
          <div className="mb-5 flex flex-wrap items-center gap-2 text-sm font-bold text-neutral-600">
            <Link href={marketBase} className="hover:text-black">THEVULGO · Barcelona</Link>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16">
        ...
        <div className="mt-6 flex flex-wrap gap-2">{getMarketConfig("barcelona").districts.map((district) => <span key={district} className="rounded-full border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm font-semibold">{district}</span>)}</div>
      </section>
