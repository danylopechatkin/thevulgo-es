...
import { BARCELONA_DISTRICTS, humanizeServicePath } from "@/lib/cities";
...
export default function BarcelonaLanding({ locale, servicePath }: { locale: string; servicePath?: string }) {
  ...
  const title = isHome
    ? isEs ? "Servicios de manitas en Barcelona" : "Handyman services in Barcelona"
    : `${serviceName} ${isEs ? "en" : "in"} Barcelona`;
  const estimateHref = `/${locale}/estimate?market=barcelona${servicePath ? `&service=${encodeURIComponent(servicePath)}` : ""}`;
  const whatsapp = `https://wa.me/${phone}?text=${encodeURIComponent(isEs ? `Hola, necesito ${serviceName.toLowerCase()} en Barcelona. Zona: ` : `Hi, I need ${serviceName.toLowerCase()} in Barcelona. Area: `)}`;
  ...
  const serviceJsonLd = {
    ...
    "@type": "Service",
    name: title,
    url: `https://www.thevulgo.es/${locale}/barcelona${servicePath ? `/${servicePath}` : ""}`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Barcelona", item: `https://www.thevulgo.es/${locale}/barcelona` },
      ...(servicePath ? [{ "@type": "ListItem", position: 3, name: serviceName, item: `https://www.thevulgo.es/${locale}/barcelona/${servicePath}` }] : []),
    ],
  };
      ...
      <section className="relative overflow-hidden border-b border-yellow-200 bg-[radial-gradient(circle_at_top_right,_#fef08a_0,_#fff_42%)]">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:py-20">
          <div className="mb-5 flex flex-wrap items-center gap-2 text-sm font-bold text-neutral-600">
            <Link href={`/${locale}/barcelona`} className="hover:text-black">THEVULGO · Barcelona</Link>
            {servicePath && <><span>/</span><span>{serviceName}</span></>}
          </div>
          <div className="max-w-4xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-yellow-300 bg-yellow-100 px-4 py-2 text-sm font-extrabold"><MapPin className="h-4 w-4" /> Barcelona capital · 10 distritos</div>
            <h1 className="text-4xl font-black tracking-tight sm:text-6xl">{title}</h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-neutral-700">
              {isEs
                ? `${serviceName} con presupuesto claro, comunicación rápida y trabajo limpio. Atendemos viviendas, pisos de alquiler, oficinas y pequeños negocios en Barcelona.`
                : `${serviceName} with clear pricing, fast communication and clean work. We serve homes, rental apartments, offices and small businesses across Barcelona.`}
            </p>
      ...
      {isHome ? (
        <>
          <section className="mx-auto max-w-7xl px-4 py-16">
            <h2 className="text-3xl font-black">{isEs ? "Servicios disponibles en Barcelona" : "Services available in Barcelona"}</h2>
            <p className="mt-3 max-w-3xl text-neutral-600">{isEs ? "El mismo catálogo de servicios de THEVULGO, organizado para reservar y medir resultados específicamente en Barcelona." : "The same THEVULGO service catalogue, organised for booking and measuring results specifically in Barcelona."}</p>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {categoryCards.map(([path, en, es]) => <Link key={path} href={`/${locale}/barcelona/${path}`} className="group rounded-3xl border border-neutral-200 p-6 shadow-sm transition hover:-translate-y-1 hover:border-yellow-300 hover:shadow-md"><Wrench className="h-7 w-7 text-yellow-500" /><h3 className="mt-4 text-xl font-extrabold">{isEs ? es : en}</h3><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold">{isEs ? "Ver servicios" : "View services"}<ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span></Link>)}
            </div>
          </section>
          <section className="border-y border-neutral-200 bg-neutral-50">
            <div className="mx-auto max-w-7xl px-4 py-16">
              ...
              <div className="mt-8 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {Object.entries(grouped).map(([group, routes]) => <div key={group}><h3 className="mb-3 text-lg font-extrabold capitalize">{group.replaceAll("-", " ")}</h3><div className="space-y-2">{routes.map((route) => <Link key={route.path} href={`/${locale}/barcelona/${route.path}`} className="flex items-center gap-2 text-sm font-semibold text-neutral-700 hover:text-black"><ArrowRight className="h-3.5 w-3.5 text-yellow-500" />{humanizeServicePath(route.path, locale)}</Link>)}</div></div>)}
              </div>
            </div>
          </section>
        </>
        ...
        <>
            ...
            <div>
              ...
              <div className="mt-8 space-y-5">
                {[
                  ...
                  isEs ? "Acordamos precio y horario antes de la visita en Barcelona." : "We agree the price and time before the Barcelona visit.",
          ...
          {related.length > 0 && <section className="border-y border-neutral-200 bg-neutral-50"><div className="mx-auto max-w-7xl px-4 py-14"><h2 className="text-2xl font-black">{isEs ? "Servicios relacionados en Barcelona" : "Related services in Barcelona"}</h2><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{related.map((route) => <Link key={route.path} href={`/${locale}/barcelona/${route.path}`} className="flex items-center justify-between rounded-2xl border border-neutral-200 bg-white p-4 font-bold shadow-sm hover:border-yellow-300">{humanizeServicePath(route.path, locale)}<ArrowRight className="h-4 w-4" /></Link>)}</div></div></section>}
        </>
      )}

      <section className="mx-auto max-w-7xl px-4 py-16">
        <h2 className="text-3xl font-black">{isEs ? "Cobertura en Barcelona" : "Barcelona coverage"}</h2>
        ...
        <div className="mt-6 flex flex-wrap gap-2">{BARCELONA_DISTRICTS.map((district) => <span key={district} className="rounded-full border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm font-semibold">{district}</span>)}</div>
      </section>

      <section className="bg-neutral-950 text-white"><div className="mx-auto max-w-5xl px-4 py-16 text-center"><h2 className="text-3xl font-black">{isEs ? `¿Necesitas ${serviceName.toLowerCase()} en Barcelona?` : `Need ${serviceName.toLowerCase()} in Barcelona?`}</h2><p className="mx-auto mt-4 max-w-2xl text-neutral-300">{isEs ? "Prepara tu solicitud en el calculador o envíanos fotos por WhatsApp." : "Build your request in the estimator or send us photos by WhatsApp."}</p><Link href={estimateHref} className="mt-8 inline-flex items-center gap-2 rounded-2xl bg-yellow-400 px-7 py-4 font-extrabold text-black">{isEs ? "Empezar presupuesto" : "Start estimate"}<ArrowRight className="h-5 w-5" /></Link></div></section>
    </main>
  );
}
