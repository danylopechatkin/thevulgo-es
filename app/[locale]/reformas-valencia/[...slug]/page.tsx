...
export function generateStaticParams() { return RENOVATION_CATEGORIES.flatMap((category) => [{ locale: "es", slug: [category.slug.es] }, ...category.services.filter((service) => !service.existingPath).map((service) => ({ locale: "es", slug: [category.slug.es, service.slug.es] }))]); }
