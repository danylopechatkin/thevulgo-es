...
import { BookOpen, Camera, Cable, ChevronDown, Hammer, KeyRound, Menu, MessageCircle, Network, Radio, ShieldCheck, Siren, Tv, Wind, Wrench, X } from "lucide-react";
...
export default function MobileHeaderMenu({ locale }: Props) {
  ...
  const technical = [
    ["cctv", "CCTV", Camera], ["redes", isEs ? "WiFi y Redes" : "WiFi & Networks", Network], ["fiber", isEs ? "Fibra" : "Fiber", Cable], ["control-de-acceso", isEs ? "Control de Acceso" : "Access Control", KeyRound], ["intercom", isEs ? "Videoporteros" : "Intercoms", Radio], ["alarmas", isEs ? "Alarmas" : "Alarms", Siren], ["seguridad-comercial", isEs ? "Negocios" : "Business Systems", ShieldCheck],
  ] as const;
  const renovations = isEs
    ? [["Electricidad e iluminación", "electricidad"], ["Paredes y acabados", "paredes-techos"], ["Cocinas y baños", "cocinas"], ["Reformas integrales", "reformas-integrales"]]
    : [["Electrical & lighting", "electrical"], ["Walls & finishes", "walls-ceilings"], ["Kitchens & bathrooms", "kitchens"], ["Full renovations", "full-renovations"]];
      ...
      <div className="relative mx-3 mt-2 max-h-[calc(100dvh-84px)] overflow-y-auto rounded-3xl border border-neutral-200 bg-white p-4 shadow-2xl">
        ...
        <nav className="space-y-2" aria-label={isEs ? "Menú móvil" : "Mobile menu"}>
          ...
          {!cityMarket ? <details className="group rounded-2xl border border-yellow-200 bg-yellow-50/60">
            ...
            <div className="border-t border-yellow-100 p-2">{technical.map(([slug, label, Icon]) => <Link key={slug} href={localizedPath(locale, `services/${slug}`)} onClick={close} className="flex min-h-12 items-center gap-3 rounded-xl px-3 py-2 text-sm font-bold text-neutral-700 hover:bg-white"><Icon className="h-4 w-4 shrink-0 text-yellow-600" />{label}</Link>)}<Link href={localizedPath(locale, "services/security-networks")} onClick={close} className="mt-1 flex min-h-12 items-center rounded-xl bg-black px-4 text-sm font-black text-white">{isEs ? "Ver todo Seguridad y Redes" : "View all Security & Networks"}</Link></div>
          </details> : null}
          ...
          {!cityMarket ? <details className="group rounded-2xl border border-neutral-200">
            ...
            <div className="border-t border-neutral-100 p-2">{renovations.map(([label, slug]) => <Link key={slug} href={`${renovationBase}/${slug}`} onClick={close} className="flex min-h-12 items-center rounded-xl px-3 py-2 text-sm font-bold text-neutral-700 hover:bg-neutral-50">{label}</Link>)}<Link href={renovationBase} onClick={close} className="mt-1 flex min-h-12 items-center rounded-xl bg-yellow-400 px-4 text-sm font-black text-black">{isEs ? "Ver todas las reformas" : "View all renovations"}</Link></div>
          </details> : null}
